...
export default function AppointmentDetailsModal({
  ...
  const [attendanceStatus, setAttendanceStatus] = useState(appointment.attendanceStatus ?? (appointment.status === 'Did Not Attend' ? 'Did Not Attend' : appointment.status === 'Completed' ? 'Attended' : ''))
  const [completionStatus, setCompletionStatus] = useState(appointment.completionStatus ?? (appointment.status === 'Completed' ? 'Completed' : ''))
  const [followUpActions, setFollowUpActions] = useState(appointment.followUpActions ?? '')
  const [additionalDocuments, setAdditionalDocuments] = useState(appointment.additionalDocumentsRequired ?? '')
  const [reportPreparationReady, setReportPreparationReady] = useState(appointment.reportPreparationReadiness === 'Ready')
  const [rescheduleReason, setRescheduleReason] = useState(appointment.rescheduleReason ?? '')
  ...
  const [createRescheduleTask, setCreateRescheduleTask] = useState(false)

  const { cases, addTask } = usePrototypeData()
  ...
  const saveAppointment = () => {
    ...
    if (mode === 'reschedule' && !rescheduleReason.trim()) return setError('Enter a rescheduling reason before saving the new appointment slot.')
    const next = withHistory({
      ...
      rescheduleReason: mode === 'reschedule' ? rescheduleReason.trim() : appointment.rescheduleReason,
    }, mode === 'reschedule' ? 'Appointment rescheduled' : 'Appointment updated', `${formatDate(date)} · ${start}–${end} · ${doctor}${mode === 'reschedule' ? ` · Reason: ${rescheduleReason.trim()}` : ''}`)
    onUpdate(next)
    setError('')
    setMode('view')
  }
  ...
  const cancelAppointment = () => {
    if (!cancellationReason) return setError('Select a cancellation reason before confirming.')
    const detail = `Reason: ${cancellationReason}. Cancelled by: ${role.name}. Follow-up required: ${followUpRequired ? 'Yes' : 'No'}.`
    onUpdate(withHistory({ ...appointment, status: 'Cancelled', cancellationReason, cancelledBy: role.name, followUpRequired }, 'Appointment cancelled', detail))
    if (createRescheduleTask) {
      addTask({ id:`TASK-${Date.now()}`, title:'Reschedule cancelled appointment', caseRef:appointment.caseRef, owner:relatedCase?.owner || role.name, dueDate:'07 Sep 2026', priority:'High', status:'Not Started', createdBy:role.name, description:`Reschedule ${appointment.patient} after cancellation. ${detail}`, taskType:'Follow-up', notifyOwner:true })
    }
    setConfirmAction(null)
    setCancellationReason('')
    setFollowUpRequired(false)
    setCreateRescheduleTask(false)
    setError('')
  }

  const saveOutcome = () => {
    if (!attendanceStatus) return setError('Select an attendance status before saving the outcome.')
    if (!completionStatus) return setError('Select an appointment completion status before saving the outcome.')
    if (!outcome.trim()) return setError('Enter an appointment outcome before saving.')
    const detailedOutcome = `Attendance: ${attendanceStatus}. Completion: ${completionStatus}. Outcome: ${outcome.trim()}. Follow-up: ${followUpActions.trim() || 'None recorded'}. Additional documents: ${additionalDocuments.trim() || 'None required'}. Report preparation readiness: ${reportPreparationReady ? 'Ready' : 'Not ready'}.`
    const nextStatus = attendanceStatus === 'Did Not Attend' ? 'Did Not Attend' : completionStatus === 'Completed' ? 'Completed' : appointment.status
    onUpdate(withHistory({ ...appointment, status: nextStatus as Appointment['status'], outcome: outcome.trim(), attendanceStatus, completionStatus, followUpActions: followUpActions.trim(), additionalDocumentsRequired: additionalDocuments.trim(), reportPreparationReadiness: reportPreparationReady ? 'Ready' : 'Not ready' }, 'Appointment outcome recorded', detailedOutcome))
    setError('')
    setMode('view')
  }
    ...
    <Modal title={mode === 'view' ? 'Appointment details' : mode === 'reschedule' ? 'Reschedule appointment' : mode === 'outcome' ? 'Add appointment outcome' : 'Edit appointment'} description={`${appointment.id} · ${appointment.patient}`} onClose={onClose} width="max-w-3xl">
      ...
      {mode === 'view' ? (
        <div className="space-y-4">
          ...
          {confirmAction && <div className="border border-amber-200 bg-amber-50 rounded-xl p-3"><p className="text-sm font-medium text-amber-900">Confirm {confirmAction === 'cancel' ? 'appointment cancellation' : confirmAction === 'attended' ? 'attendance' : 'did not attend'}?</p><p className="text-xs text-amber-800 mt-1">This updates the appointment status and records the change in history.</p>{confirmAction==='cancel'&&<div className="mt-3 grid sm:grid-cols-2 gap-3"><div><label className={label}>Cancellation reason *</label><select className={field} value={cancellationReason} onChange={e=>setCancellationReason(e.target.value)}><option value="">Select reason</option><option>Patient requested cancellation</option><option>Doctor unavailable</option><option>Client requested cancellation</option><option>Scheduling conflict</option><option>Other</option></select></div><div><label className={label}>Cancelled by</label><input className={`${field} bg-white/60`} value={role.name} readOnly/></div><label className="flex items-center gap-2 text-xs text-amber-900"><input type="checkbox" checked={followUpRequired} onChange={e=>setFollowUpRequired(e.target.checked)}/> Follow-up required</label><label className="flex items-center gap-2 text-xs text-amber-900"><input type="checkbox" checked={createRescheduleTask} onChange={e=>setCreateRescheduleTask(e.target.checked)}/> Create rescheduling task</label></div>}<div className="flex justify-end gap-2 mt-3"><button onClick={() => {setConfirmAction(null);setCancellationReason('');setFollowUpRequired(false);setCreateRescheduleTask(false)}} className="px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-white/60 rounded-lg">Back</button><button disabled={confirmAction==='cancel'&&!cancellationReason} onClick={() => confirmAction === 'cancel' ? cancelAppointment() : confirmAction === 'attended' ? applyStatus('Completed', 'Marked attended') : applyStatus('Did Not Attend', 'Marked did not attend')} className="px-3 py-1.5 text-xs font-medium text-white bg-brand-600 hover:bg-brand-700 disabled:opacity-40 rounded-lg">Confirm</button></div></div>}
        ...
        <div className="space-y-4">
          <div className="grid sm:grid-cols-2 gap-3"><div><label className={label}>Attendance status *</label><select className={field} value={attendanceStatus} onChange={e=>setAttendanceStatus(e.target.value)}><option value="">Select attendance</option><option>Attended</option><option>Did Not Attend</option><option>Cancelled on arrival</option></select></div><div><label className={label}>Appointment completion status *</label><select className={field} value={completionStatus} onChange={e=>setCompletionStatus(e.target.value)}><option value="">Select completion status</option><option>Completed</option><option>Partially completed</option><option>Unable to complete</option></select></div></div>
          <div><label className={label}>Outcome summary *</label><textarea rows={4} className={field} value={outcome} onChange={(e) => setOutcome(e.target.value)} placeholder="Record the appointment outcome and relevant clinical/admin result..." /></div>
          <div className="grid sm:grid-cols-2 gap-3"><div><label className={label}>Follow-up actions</label><textarea rows={3} className={field} value={followUpActions} onChange={e=>setFollowUpActions(e.target.value)} placeholder="e.g. Follow-up examination in 6 weeks"/></div><div><label className={label}>Additional documents required</label><textarea rows={3} className={field} value={additionalDocuments} onChange={e=>setAdditionalDocuments(e.target.value)} placeholder="e.g. GP notes, imaging or employment records"/></div></div>
          <label className="flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-700"><input type="checkbox" checked={reportPreparationReady} onChange={e=>setReportPreparationReady(e.target.checked)}/> Report preparation is ready to begin</label>
        ...
        <div className="space-y-4">
          ...
          {mode==='reschedule'&&<><div><label className={label}>Rescheduling reason *</label><textarea rows={3} className={field} value={rescheduleReason} onChange={e=>setRescheduleReason(e.target.value)} placeholder="Record why the existing appointment is being moved…"/></div><div className={`rounded-lg border px-3 py-2.5 text-xs ${doctorConflict?'border-red-200 bg-red-50 text-red-700':'border-teal-200 bg-teal-50 text-teal-700'}`}><strong>Doctor availability:</strong> {doctorConflict?`${doctor} has a conflicting appointment at ${doctorConflict.time}.`:`${doctor} is available for the selected date and time in the current diary.`}</div></>}
