...
export default function NewBookingModal({
  ...
  const steps = ['Booking & patient', 'Client, doctor & speciality', 'Appointment requirements', 'Documents', 'Fees & deadlines', 'Review & confirmation']

  const selectedDoctor = activeDoctors.find((item) => item.name === doctor)
  ...
  const bookingMatch = useMemo(
    () => existingBookings.find(
      (b) => patient.trim() && b.patient.toLowerCase() === patient.trim().toLowerCase() && b.caseType === caseType && b.status !== 'Cancelled' && b.status !== 'Converted to Case'
    ),
    [existingBookings, patient, caseType]
  )
  ...
  const validateStep2 = () => {
    if (!doctor.trim()) {
      setError('Assigned doctor / medical expert is required. Client is optional.')
      return false
    }
    setError('')
    return true
  ...
  const buildBooking = (status: Booking['status']): Booking => ({
    ...
    appointmentDate: '—',
    appointmentRequired: appointmentRequired === 'Yes',
    preferredAppointmentDate: appointmentRequired === 'Yes' ? preferredDate : undefined,
    status,
    owner: 'Unassigned',
    priority,
    source,
    caseType,
    missingInformation: !patient.trim() || !doctor.trim() ? 'Yes' : 'No',
    notes: notes.trim(),
      ...
      {step === 1 && (
        <div className="space-y-4">
          <div>
            ...
            <select className={field} value={patient} onChange={(e) => { setPatient(e.target.value); setError('') }}>
            ...
            <p className="mt-1 text-[11px] text-slate-400">Patient = the person being medically assessed. Select the master record so the booking, case, appointments and reports stay linked.</p>
          </div>
          {patientMatch && (
      ...
      {step === 2 && (
        <div className="space-y-4">
          <div>
            ...
            <p className="mt-1 text-[11px] text-slate-400">Client = the solicitor, insurer or organisation that instructs Evaluate. It is separate from the patient.</p>
          </div>
          {bookingMatch && (
            <div className="flex gap-2.5 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5">
              <AlertTriangle size={16} className="text-amber-600 shrink-0 mt-0.5" />
              <div className="text-xs text-amber-800">
                ...
                <p className="mt-0.5">An active booking already exists for this patient and case type: {bookingMatch.ref} ({bookingMatch.status}).</p>
              </div>
            </div>
          )}
          <div className="grid sm:grid-cols-3 gap-3">
            ...
            <div>
              <label className={label}>Medical speciality</label>
              <input className={`${field} bg-slate-50`} value={selectedDoctor?.speciality ?? 'Select a doctor'} disabled />
            </div>
            <div>
      ...
      {step === 6 && (
        <div className="space-y-3 text-sm">
          <div className="bg-slate-50 rounded-lg border border-slate-100 p-4 grid grid-cols-2 gap-x-6 gap-y-2">
            ...
            <div className="flex justify-between gap-3"><span className="text-slate-500">Client</span><span className="text-slate-700 text-right">{client || 'Direct / no client'}</span></div>
            <div className="flex justify-between gap-3"><span className="text-slate-500">Doctor</span><span className="text-slate-700 text-right">{doctor || 'Required'}</span></div>
            <div className="flex justify-between gap-3"><span className="text-slate-500">Appointment requirement</span><span className="text-slate-700 text-right">{appointmentRequired === 'Yes' ? (preferredDate || 'Date required') : 'Not required'}</span></div>
