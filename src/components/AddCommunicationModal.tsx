...
export default function AddCommunicationModal({ onClose, defaultCaseRef = '', defaultRecipient = '', defaultType = 'Email' }: { onClose: () => void; defaultCaseRef?: string; defaultRecipient?: string; defaultType?: CommunicationItem['type'] }) {
  const { communications, tasks, cases, addCommunication, addTask } = usePrototypeData()
  const { role } = useRole()
  const { showToast } = useToast()
  const [caseRef, setCaseRef] = useState(defaultCaseRef || cases[0]?.ref || '')
  const [type, setType] = useState<CommunicationItem['type']>(defaultType)
  ...
  const [communicationTime, setCommunicationTime] = useState(() => new Date().toTimeString().slice(0, 5))
  ...
  const save = () => {
    if (!caseRef || !from.trim() || !to.trim() || !subject.trim() || !summary.trim() || !communicationDate || !communicationTime) {
      setError('Case, sender, recipient, date, time, subject and notes are required.')
      return
    }
    ...
    addCommunication({
      ...
      from: from.trim(),
      to: to.trim(),
      date: `${formatDate(communicationDate)} · ${communicationTime}`,
    ...
    <Modal title="Add communication" description="Record case communication, visibility and any required follow-up." onClose={onClose} width="max-w-2xl">
      <div className="space-y-4">
        ...
        <div className="grid md:grid-cols-2 gap-3">
          ...
          <div><label className={label}>Date *</label><input type="date" className={field} value={communicationDate} onChange={(e) => { setCommunicationDate(e.target.value); setError('') }} /></div>
          <div><label className={label}>Time *</label><input type="time" className={field} value={communicationTime} onChange={(e) => { setCommunicationTime(e.target.value); setError('') }} /></div>
        </div>
