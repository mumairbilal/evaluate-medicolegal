...
export default function NewPatientModal({
  ...
  const matchingFields = useMemo(() => {
    if (!possibleMatch) return [] as string[]
    const fields: string[] = []
    const n = normalise(name)
    const d = normalisePatientDob(dob)
    const e = normalise(email)
    const ph = phone.replace(/\D/g, '')
    const a = normalise(address)
    if (n && normalise(possibleMatch.name) === n) fields.push('Name')
    if (d && normalisePatientDob(possibleMatch.dob) === d) fields.push('Date of birth')
    if (e && normalise(possibleMatch.email) === e) fields.push('Email')
    if (ph && possibleMatch.phone.replace(/\D/g, '') === ph) fields.push('Phone')
    if (a && possibleMatch.address && normalise(possibleMatch.address) === a) fields.push('Address')
    return fields
  }, [possibleMatch, name, dob, email, phone, address])
  ...
  const handleSubmit = () => {
    ...
    onCreate({
      ...
      patientWarnings: patientWarnings.trim() || undefined,
  ...
  return (
    <Modal title="Create patient" description="Add a new patient record. Possible duplicates are checked as you enter details." onClose={onClose}>
      <div className="space-y-4">
        ...
        {possibleMatch && (
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
            ...
            <div className="mt-3 rounded-lg bg-white border border-amber-100 p-3 text-sm">
              ...
              <p className="text-[11px] text-amber-700 mt-2"><span className="font-medium">Matching fields:</span> {matchingFields.join(', ') || 'Potential identifying match'}</p>
            </div>
            <div className="flex flex-wrap gap-2 mt-3">
              {onViewExisting && <button onClick={() => onViewExisting(possibleMatch)} className="px-3 py-1.5 text-xs font-medium text-brand-700 border border-brand-200 bg-white hover:bg-brand-50 rounded-lg">View existing profile</button>}
        ...
        <div>
          <label className={label}>Patient warnings</label>
          <textarea className={field} rows={2} value={patientWarnings} onChange={(e) => setPatientWarnings(e.target.value)} placeholder="Operational or safety warnings visible to authorised case users" />
        </div>
        <div>
          <label className={label}>Additional notes</label>
          <textarea className={field} rows={3} value={additionalNotes} onChange={(e) => setAdditionalNotes(e.target.value)} placeholder="Optional patient context or notes" />
        </div>
      </div>
