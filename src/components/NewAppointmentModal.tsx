...
export default function NewAppointmentModal({
  ...
  const { doctors, cases } = usePrototypeData()
  ...
  const handleSubmit = () => {
    if (!caseRef || !doctor || !date || !startTime || !endTime) {
      setError('Case, doctor, date, start time and end time are required.')
      return
    }
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select className={field} value={doctor} onChange={(e) => setDoctor(e.target.value)}>
              <option value="">Select doctor</option>{doctors.filter((item) => item.status === 'Active').map((item) => <option key={item.id} value={item.name}>{item.name} · {item.speciality} · {item.availability}</option>)}
            </select>
          </div>
        </div>
