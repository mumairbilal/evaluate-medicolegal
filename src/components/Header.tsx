...
import { notificationVisibleTo, useNotifications } from '../context/NotificationContext'
import { useProfilePhoto } from '../context/ProfilePhotoContext'

const createDestinations: Record<string, string> = {
  'New booking': '/bookings?new=1',
  'New case': '/cases?new=1',
  'New patient': '/patients?new=1',
  'New appointment': '/calendar?new=1',
  'New task': '/tasks?new=1',
  'Upload document': '/documents?new=1',
  'Add communication': '/communication?new=1',
}

const createRequiredModule: Record<string, string> = {
...
function contextualCreateDestination(item: string, pathname: string) {
  const base = createDestinations[item] ?? '/dashboard'
  const caseMatch = pathname.match(/^\/cases\/([^/]+)/)
  const caseRef = caseMatch?.[1]
  if (!caseRef || !['New appointment', 'New task', 'Upload document', 'Add communication'].includes(item)) return base
  const separator = base.includes('?') ? '&' : '?'
  return `${base}${separator}case=${encodeURIComponent(caseRef)}`
}
...
export default function Header({ onOpenMobileNav }: { onOpenMobileNav?: () => void }) {
  ...
  const roleNotifications = notifications.filter((item) => notificationVisibleTo(item, role.id, role.name))
  const unreadNotifications = roleNotifications.filter((item) => !item.read)
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="relative" ref={createRef}>
          ...
          {createOpen && (
            <div className="absolute right-0 mt-2 w-56 bg-white z-50 border border-slate-200 rounded-xl shadow-xl shadow-slate-200/70 py-1.5 text-sm">
              {['New booking', 'New case', 'New patient', 'New appointment', 'New task', 'Upload document', 'Add communication'].filter((item) => role.nav.some((section) => section.items.some((navItem) => navItem.to === createRequiredModule[item]))).map((item) => (
                <button
                  key={item}
                  className="w-full text-left px-4 py-2 hover:bg-brand-50/60 hover:text-brand-700 text-slate-700"
                  onClick={() => {
                    setCreateOpen(false)
                    navigate(contextualCreateDestination(item, location.pathname))
                  }}
                >
                  {item}
                </button>
        ...
        <div className="relative" ref={notifRef}>
          ...
          >
            ...
            {unreadCount > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] min-w-4 h-4 px-1 rounded-full flex items-center justify-center">{unreadCount > 9 ? '9+' : unreadCount}</span>}
          </button>
          {notifOpen && (
            <div className="absolute right-0 mt-2 w-[calc(100vw-2rem)] max-w-80 bg-white z-50 border border-slate-200 rounded-xl shadow-xl shadow-slate-200/70 py-2 text-sm">
              ...
              {unreadNotifications.length === 0 && <div className="px-4 py-6 text-center text-xs text-slate-400"><p className="font-medium text-slate-600">You're all caught up</p><p className="mt-1">No unread notifications for this role.</p></div>}
