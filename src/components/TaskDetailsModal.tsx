...
export default function TaskDetailsModal({ taskId, onClose }: { taskId: string; onClose: () => void }) {
  ...
  const [dueDate, setDueDate] = useState(task ? toInputDate(task.dueDate) : '')

  if (!task || !draft) return null

  const save = () => {
    if (!draft.title.trim() || !dueDate) { showToast('Task title and due date are required.'); return }
    ...
    updateTask(task.id, { ...draft, dueDate: formatDate(dueDate), title: draft.title.trim(), description: draft.description?.trim(), activity })
  ...
  const markComplete = () => {
    if (!dueDate) { showToast('Due date is required before completing the task.'); return }
    const completed = {
      ...draft,
      status: 'Completed' as const,
      dueDate: formatDate(dueDate),
      activity: [{ id: `ACT-${Date.now()}-complete`, date: 'Just now', user: draft.owner, action: 'Task marked complete' }, ...(draft.activity ?? [])],
    }
    updateTask(task.id, completed)
    showToast(`Task "${draft.title}" marked complete.`)
    onClose()
  }
  ...
  return (
    <Modal title="Task details" description={`${task.id} · ${task.caseRef}`} onClose={onClose} width="max-w-3xl">
      <div className="grid lg:grid-cols-[minmax(0,1fr)_260px] gap-5">
        <div className="space-y-4">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            ...
            <div><label className={label}>Reassign owner</label><select className={field} value={draft.owner} onChange={(e) => setDraft({ ...draft, owner: e.target.value })}><option>Unassigned</option>{roles.map((entry) => <option key={entry.id} value={entry.name}>{entry.name}</option>)}</select></div>
            <div><label className={label}>Due date *</label><input type="date" className={field} value={dueDate} onChange={(e) => setDueDate(e.target.value)} /></div>
          ...
          <div><label className={label}>Related document</label><select className={field} value={draft.supportingDocument ?? ''} onChange={(e) => setDraft({ ...draft, supportingDocument: e.target.value })}><option value="">No related document</option>{relatedDocs.map((doc) => <option key={doc.id} value={doc.name}>{doc.name}</option>)}</select></div>
      ...
      <div className="flex justify-between gap-2 mt-5 pt-4 border-t border-slate-100"><button onClick={markComplete} disabled={draft.status === 'Completed'} className="px-4 py-2 text-sm font-medium border border-teal-200 bg-teal-50 text-teal-700 rounded-lg hover:bg-teal-100 disabled:opacity-50">Mark complete</button><div className="flex gap-2"><button onClick={onClose} className="px-3 py-2 text-sm text-slate-600 hover:bg-slate-50 rounded-lg">Cancel</button><button onClick={save} className="px-4 py-2 text-sm font-medium text-white bg-brand-600 rounded-lg hover:bg-brand-700">Save changes</button></div></div>
    </Modal>
  )
}
