...
export default function QaReviewModal({ qaId, onClose }: { qaId: string; onClose: () => void }) {
  const { qaQueue, updateQaItem, reports, updateReport, documents, cases, updateCase } = usePrototypeData()
  const { role } = useRole()
  ...
  const saveReview = () => {
    if (!canReview) { showToast('QA review actions are restricted to the Quality Assurance role.'); return }
  ...
  const addGeneralComment = () => {
    if (!canReview) { showToast('QA comments are restricted to the Quality Assurance role.'); return }
  ...
  const reassign = () => {
    if (!canReview) { showToast('QA reviewer assignment is restricted to the Quality Assurance role.'); return }
    updateQaItem(item.id, (current) => ({
      ...current,
      reviewer,
  ...
  const returnForAmendments = () => {
    if (!canReview) { showToast('Only Quality Assurance can return a report for amendments.'); setConfirmAction(null); return }
    ...
    updateCase(item.caseRef, (current) => transitionCaseForEvent(current, 'qa-returned', role.name, `${issues.length} QA amendment${issues.length === 1 ? '' : 's'} required`))
    pushNotification({ title: 'QA decision recorded — amendments requested', caseRef: item.caseRef, type: 'QA amendment requested', priority: item.priority, actionPath: `/quality-assurance?case=${encodeURIComponent(item.caseRef)}`, detail: `You returned ${item.reportType} with ${issues.length} amendment${issues.length === 1 ? '' : 's'}. The QA request is now waiting for a revised report.`, recipientRoleIds: ['quality-assurance'], recipientNames: [role.name], dedupeKey: `workflow:qa:${item.caseRef}` })
    pushNotification({ title: 'QA amendments require action', caseRef: item.caseRef, type: 'QA amendment requested', priority: item.priority, actionPath: report?.id ? `/reports?report=${encodeURIComponent(report.id)}` : `/cases/${item.caseRef}?tab=Reports`, detail: `${role.name} returned ${item.reportType} with ${issues.length} amendment${issues.length === 1 ? '' : 's'}. Resolve the recorded QA comments and resubmit the report.`, recipientRoleIds: ['medical-expert'], recipientNames: [item.doctor], dedupeKey: `workflow:doctor:${item.caseRef}` })
    setConfirmAction(null)
    showToast(`Report ${item.caseRef} returned for amendments.`)
  }

  const approve = () => {
    if (!canReview) { showToast('Only Quality Assurance can approve a QA review.'); setConfirmAction(null); return }
    ...
    updateCase(item.caseRef, (current) => transitionCaseForEvent(current, 'qa-approved', role.name, 'QA review approved; awaiting final medical approval'))
    pushNotification({ title: 'QA review approved', caseRef: item.caseRef, type: 'Report approved', priority: item.priority, actionPath: `/quality-assurance?case=${encodeURIComponent(item.caseRef)}`, detail: `You approved ${item.reportType}. QA is complete and the case is now awaiting final medical approval.`, recipientRoleIds: ['quality-assurance'], recipientNames: [role.name], dedupeKey: `workflow:qa:${item.caseRef}` })
    pushNotification({ title: 'QA approved — final approval required', caseRef: item.caseRef, type: 'Final approval required', priority: item.priority, actionPath: report?.id ? `/reports?report=${encodeURIComponent(report.id)}` : `/cases/${item.caseRef}?tab=Reports`, detail: `Quality Assurance approved ${item.reportType}. ${item.doctor} can now complete final medical approval.`, recipientRoleIds: ['medical-expert'], recipientNames: [item.doctor], dedupeKey: `workflow:doctor:${item.caseRef}` })
    setConfirmAction(null)
    ...
    <Modal title="QA review workspace" description={`${item.caseRef} · ${item.patient} · ${item.reportType}`} onClose={onClose} width="max-w-7xl">
      <div className="flex items-center justify-between gap-3 flex-wrap mb-4 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5">
        ...
        <div className="flex gap-1.5 flex-wrap">
          <button disabled={!canReview} onClick={saveReview} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium border border-slate-200 bg-white rounded-lg hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed"><Save size={13} /> Save draft review</button>
          <button disabled={!canReview} onClick={() => setConfirmAction('return')} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium border border-amber-200 bg-amber-50 text-amber-700 rounded-lg hover:bg-amber-100 disabled:opacity-40 disabled:cursor-not-allowed"><RotateCcw size={13} /> Return for amendments</button>
          <button disabled={!canReview} onClick={() => setConfirmAction('approve')} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-brand-600 text-white rounded-lg hover:bg-brand-700 disabled:opacity-40 disabled:cursor-not-allowed"><CheckCircle2 size={13} /> Approve report</button>
        </div>
      </div>
        ...
        <div className="space-y-3">
          ...
          <Section title="Reviewer assignment" compact>
            ...
            <select className={field} value={reviewer} onChange={(e) => setReviewer(e.target.value)}>
              ...
              {roles.filter((entry) => entry.id === 'quality-assurance').map((entry) => <option key={entry.id} value={entry.name}>{entry.name}</option>)}
            </select>
            <button onClick={reassign} className="mt-2 w-full rounded-lg border border-brand-200 bg-brand-50 px-3 py-2 text-xs font-semibold text-brand-700 hover:bg-brand-100">Assign reviewer / Reassign review</button>
          </Section>

          <Section title="Comments" compact>
            <div className="flex gap-2 mb-3"><input className={field} value={generalComment} onChange={(e) => setGeneralComment(e.target.value)} placeholder="General QA comment..." /><button onClick={addGeneralComment} className="px-3 py-2 text-xs font-medium bg-slate-800 text-white rounded-lg">Add comment</button></div>
