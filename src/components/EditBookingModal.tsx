...
export default function EditBookingModal({
  ...
  const patientOptions = useMemo(() => loadPatients(seedPatients).filter((item) => item.status === 'Active'), [])
  ...
  const save = () => {
    if (!patient.trim() || !doctor.trim() || doctor === 'Unassigned') {
      setError('Patient and assigned doctor are required. Client is optional.')
      return
    }
    if (duplicate) {
      ...
      <div className="space-y-5">
        ...
        <section>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <div><label className={label}>Patient *</label><select className={field} value={patient} onChange={(e) => { setPatient(e.target.value); setError('') }}><option value="">Select patient</option>{patientOptions.map((p) => <option key={p.id} value={p.name}>{p.name} · DOB {p.dob}</option>)}</select></div>
            <div><label className={label}>Client / instructing organisation (optional)</label><select className={field} value={client === 'Direct / no instructing organisation' ? '' : client} onChange={(e) => { setClient(e.target.value); setError('') }}><option value="">Direct instruction / no client</option>{activeClients.map((c) => <option key={c.id} value={c.name}>{c.name} · {c.type}</option>)}</select></div>
        ...
        <section className="border-t border-slate-100 pt-4">
          ...
          <div className="grid sm:grid-cols-3 gap-3">
            <div><label className={label}>Assigned doctor / medical expert *</label><select className={field} value={doctor === 'Unassigned' ? '' : doctor} onChange={(e) => setDoctor(e.target.value)}><option value="">Select doctor</option>{activeDoctors.map((d) => <option key={d.id} value={d.name}>{d.name} · {d.speciality} · {d.availability}</option>)}</select></div>
