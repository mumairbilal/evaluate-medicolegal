  ...
  return (
    <Modal title="Edit patient" description="Update patient details and communication requirements." onClose={onClose}>
      <div className="space-y-4">
        ...
        <div><label className={label}>Patient warnings</label><textarea className={field} rows={2} value={draft.patientWarnings ?? ''} onChange={(e) => update('patientWarnings', e.target.value)} placeholder="Operational or safety warnings" /></div>
        <div><label className={label}>Additional notes</label><textarea className={field} rows={3} value={draft.additionalNotes ?? ''} onChange={(e) => update('additionalNotes', e.target.value)} /></div>
      </div>
