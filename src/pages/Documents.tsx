...
export default function Documents() {
  ...
  const { cases, documents, reports, addDocuments, updateDocument, removeDocument, updateCase } = usePrototypeData()
  const [view, setView] = useState<'workspace' | 'preparation'>(() => searchParams.get('view') === 'preparation' ? 'preparation' : 'workspace')
  ...
  // Deep-link support: /documents?case=REF&doc=ID opens straight to that
  // document's preview (used by document references linked from the Case page).
  useEffect(() => {
  ...
  const visibleDocuments = useMemo(() => {
    ...
    const caseFiltered = scopedCase ? categoryFiltered.filter((document) => document.caseRef === scopedCase) : categoryFiltered
    return [...caseFiltered].sort((a, b) => {
  ...
  }, [filtered, categoryNav, sort, scopedCase])
  ...
  return (
    <div className="space-y-4">
      {scopedCase && <div className="rounded-lg border border-brand-100 bg-brand-50 px-3 py-2 text-xs text-brand-800">Documents scoped to case <strong>{scopedCase}</strong></div>}
                      ...
                      <tr key={document.id} onClick={() => openDocumentPreview(document.id)} className={`cursor-pointer hover:bg-slate-50 ${selectedDocument?.id === document.id ? 'bg-brand-50/40' : ''}`}>
                        ...
                        <td className="px-3 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <div className="min-w-0"><p className="max-w-[220px] truncate font-medium text-slate-800">{document.name}</p><p className="text-xs text-slate-500"><Link to={`/cases/${document.caseRef}`} onClick={(event) => event.stopPropagation()} className="font-medium text-brand-600 hover:text-brand-700">{document.caseRef}</Link> · {document.patient}</p></div>
                          </div>
                        </td>
      ...
      {modalOpen && (
        <UploadDocumentModal
          ...
          defaultCaseRef={scopedCase}
          defaultPatient={cases.find((item) => item.ref === scopedCase)?.patient ?? ''}
          lockCase={Boolean(scopedCase)}
...
function FilePreparationWorkspace({
  ...
  cases: import('../types').CaseRecord[]
  updateCase: (ref: string, updater: import('../types').CaseRecord | ((current: import('../types').CaseRecord) => import('../types').CaseRecord)) => void
  defaultCaseRef?: string
}) {
  const casesWithDocuments = useMemo(() => cases.filter((record) => documents.some((document) => document.caseRef === record.ref)), [documents, cases])
  const [caseRef, setCaseRef] = useState(defaultCaseRef || casesWithDocuments[0]?.ref || '')
  ...
  useEffect(() => {
    if (!caseRef) return
    updateCase(caseRef, (current) => transitionCaseForEvent(current, 'file-preparation-started', uploadedBy, 'File preparation workspace opened'))
  }, [caseRef, updateCase, uploadedBy])
  ...
  const saveBundle = (markReady: boolean) => {
    ...
    addDocuments([item])
    if (markReady && caseRef) updateCase(caseRef, (current) => transitionCaseForEvent(current, 'file-ready', uploadedBy, `Prepared bundle ${version} confirmed and marked ready`))
    showToast(markReady ? 'Prepared file marked ready. Case moved to File Ready.' : `${version} saved to the document library.`)
    setBundlePreview(null)
  }
        ...
        <aside className="max-h-[650px] overflow-y-auto p-3">
          <div className="space-y-4">
            ...
            <div className="border-t border-slate-200 pt-3">
              ...
              {duplicatePairs.length ? <div className="space-y-2">{duplicatePairs.map((group, index) => <div key={index} className="rounded-lg border border-amber-200 bg-amber-50 p-2"><p className="text-[11px] font-medium text-amber-800">Possible duplicate group</p>{group.map((document, documentIndex) => <div key={document.id} className="mt-1 flex items-center justify-between gap-2"><p className="min-w-0 truncate text-[11px] text-amber-700">{document.version} · {document.name}</p>{documentIndex > 0 && <button onClick={() => { removeDocument(document.id); setOrder((current) => current.filter((id) => id !== document.id)); setIncluded((current) => current.filter((id) => id !== document.id)); if (activeDocId === document.id) setActiveDocId(group[0]?.id ?? null); showToast(`Duplicate document ${document.name} removed from the preparation set.`) }} className="shrink-0 rounded border border-amber-300 bg-white px-2 py-1 text-[10px] font-medium text-amber-800 hover:bg-amber-100">Remove duplicate</button>}</div>)}</div>)}</div> : <p className="text-[11px] text-slate-500">No duplicate file names detected in this case.</p>}
            </div>
            ...
            <div className="border-t border-slate-200 pt-3">
              ...
              <button className="flex w-full items-center justify-center gap-2 rounded-lg bg-brand-600 px-3 py-2 text-sm font-medium text-white hover:bg-brand-700" onClick={generateBundle}><Layers3 size={15} /> Merge PDF / generate bundle</button>
