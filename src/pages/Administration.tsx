...
import AdminConfigurationWorkspaces from '../components/AdminConfigurationWorkspaces'
...
const configSeed: Record<string,string[]> = {
  ...
  'Notification rules':['New assignment','Appointment created','Appointment changed','Missing document','Task approaching deadline','Task overdue','Report submitted','QA review started','QA amendment requested','Final approval required','Report approved','Report delivered','Case completed'],
...
export default function Administration() {
  ...
  const [config,setConfig]=useState<Record<string,string[]>>(()=>load('evaluate-admin-config-v2-doc-prd',configSeed))
  ...
  const [security,setSecurity]=useState(()=>load('evaluate-security-settings-v2-doc-prd',{ passwordMinLength:'10', passwordComplexity:'Uppercase, lowercase, number and symbol', mfa:'Required for administrators and QA', sessionDuration:'30 minutes', failedLogin:'5 attempts / 15 minute lockout', exportRestriction:'Permission required + confidentiality confirmation', downloadRestriction:'Role-based + audit logged', retention:'7 years', userAccessLogs:true, documentAccessLogs:true, accountDeactivation:true, secureBackups:true, secureTransmission:true, privacyControls:true }))
  ...
  return <div className="space-y-4">
    ...
    {tab==='configuration'&&<>
      ...
      <AdminConfigurationWorkspaces />
      <div className="grid lg:grid-cols-2 gap-4"><ConfigCard title="Notification rules" items={config['Notification rules'] ?? []} onChange={(next)=>{const updated={...config,'Notification rules':next};persist('evaluate-admin-config-v2-doc-prd',updated,setConfig);addAudit('Configuration changed','Configuration','—','Notification rules configuration updated.')}}/></div>
    ...
    {tab==='security'&&<section className="bg-white border border-slate-200 rounded-xl overflow-hidden max-w-5xl">
      <div className="px-5 py-4 border-b border-slate-100"><h2 className="text-base font-semibold text-slate-900">Security settings</h2><p className="text-xs text-slate-500 mt-1">Configure authentication, access logging, secure handling, session, export, download and data-retention controls.</p></div>
      <div className="p-5 grid md:grid-cols-2 gap-4">
        ...
        <SecurityToggle title="User access logs" detail="Record individual user access and important account activity." checked={security.userAccessLogs} onChange={value=>setSecurity({...security,userAccessLogs:value})}/>
        <SecurityToggle title="Document access logs" detail="Record access/download activity for sensitive case documents." checked={security.documentAccessLogs} onChange={value=>setSecurity({...security,documentAccessLogs:value})}/>
        <SecurityToggle title="Account deactivation controls" detail="Suspended/deactivated accounts cannot access operational records." checked={security.accountDeactivation} onChange={value=>setSecurity({...security,accountDeactivation:value})}/>
        <SecurityToggle title="Secure backups" detail="Keep protected backups as part of the data-protection process." checked={security.secureBackups} onChange={value=>setSecurity({...security,secureBackups:value})}/>
        <SecurityToggle title="Secure data transmission" detail="Require protected transmission for sensitive data and report delivery." checked={security.secureTransmission} onChange={value=>setSecurity({...security,secureTransmission:value})}/>
        <SecurityToggle title="Privacy & data-protection controls" detail="Apply role-limited access and approved retention/privacy processes." checked={security.privacyControls} onChange={value=>setSecurity({...security,privacyControls:value})}/>
      </div>
      <div className="px-5 py-3 border-t border-slate-100 flex items-center justify-between gap-3"><p className="text-xs text-slate-500">Security-setting changes are recorded in the audit log.</p><button onClick={()=>{localStorage.setItem('evaluate-security-settings-v2-doc-prd',JSON.stringify(security));addAudit('Security settings changed','Security','—','Authentication, session or data-control settings updated.');showToast('Security settings saved.')}} className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium bg-brand-600 text-white rounded-lg hover:bg-brand-700"><Save size={14}/> Save security settings</button></div>
    </section>}
...
function SecurityToggle({title,detail,checked,onChange}:{title:string;detail:string;checked:boolean;onChange:(value:boolean)=>void}){
  return <label className="rounded-lg border border-slate-200 p-3 flex items-start justify-between gap-4 hover:border-brand-200"><span><span className="block text-xs font-semibold text-slate-700">{title}</span><span className="block text-[11px] text-slate-500 mt-1">{detail}</span></span><input type="checkbox" checked={checked} onChange={e=>onChange(e.target.checked)} className="mt-0.5 h-4 w-4 rounded border-slate-300 text-brand-600"/></label>
}
...
function ConfigurationDetailReference(){
  const blocks = [
    {title:'Case status configuration', headers:['Status','Order','Allowed next','Role access','Required / completion'], rows:[['New Booking','1','Information Required, Booking Confirmed, On Hold, Cancelled','Booking Administration, Operations','Patient + client + booking instruction captured'],['Appointment Scheduled','5','Appointment Completed, On Hold, Cancelled','Booking Administration, Operations','Appointment exists / outcome required to progress'],['QA Review','12','Amendments Required, Awaiting Final Approval, On Hold, Cancelled','Quality Assurance','QA checklist + comments / QA decision recorded'],['Completed','19','Archived','Booking Administration, Operations, System Administration','Final delivery + all required actions/tasks complete']]},
...
function SystemFeedbackPatterns(){
  ...
  return <section className="bg-white border border-slate-200 rounded-xl p-4"><div><h3 className="text-sm font-semibold text-slate-900">System feedback and states</h3><p className="text-xs text-slate-500 mt-1">Reusable PRD global-state patterns: loading, empty, error, permission denied, unsaved changes, success, delete confirmation and processing.</p></div><div className="grid md:grid-cols-2 xl:grid-cols-4 gap-3 mt-4">
    ...
    <div className="rounded-lg border border-slate-200 bg-slate-50 p-3"><Lock size={18} className="text-slate-500"/><p className="text-xs font-semibold text-slate-700 mt-2">Permission denied</p><p className="text-[11px] text-slate-500 mt-1">You do not have permission to view this information. Sensitive fields are not partially exposed.</p><button className="text-[11px] font-medium text-slate-600 mt-2">Return</button></div>
    <div className="rounded-lg border border-amber-200 bg-amber-50/50 p-3"><Save size={18} className="text-amber-700"/><p className="text-xs font-semibold text-amber-900 mt-2">Unsaved changes</p><p className="text-[11px] text-amber-700 mt-1">You have changes that have not been saved.</p><div className="mt-2 flex flex-wrap gap-2 text-[11px] font-medium"><button className="text-brand-600">Save and continue</button><button className="text-slate-600">Leave without saving</button><button className="text-slate-500">Cancel</button></div></div>
    <div className="rounded-lg border border-red-200 bg-red-50/40 p-3"><X size={18} className="text-red-600"/><p className="text-xs font-semibold text-red-800 mt-2">Delete confirmation</p><p className="text-[11px] text-red-700 mt-1">Record name and impact warning are shown before deletion; a reason is captured where required.</p><div className="mt-2 flex gap-2 text-[11px] font-medium"><button className="text-red-700">Confirm</button><button className="text-slate-600">Cancel</button></div></div>
    <div className="rounded-lg border border-brand-200 bg-brand-50/40 p-3"><Loader2 size={18} className="text-brand-600 animate-spin"/><p className="text-xs font-semibold text-brand-900 mt-2">Processing</p><p className="text-[11px] text-brand-700 mt-1">Used for AI processing, PDF merging, uploads, report generation and exports.</p><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-brand-100"><div className="h-full w-2/3 rounded-full bg-brand-500"/></div></div>
  </div></section>
}
