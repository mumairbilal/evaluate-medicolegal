...
import type { CaseRecord, Client } from '../types'

export default function ClientDetail() {
  const { id } = useParams()
  const navigate = useNavigate()
  const { role } = useRole()
  const { cases, clients, bookings, updateClient, removeClient } = usePrototypeData()
    ...
    <header>
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
          <Link to={`/cases?new=1&client=${encodeURIComponent(client.name)}`} className="inline-flex items-center gap-1.5 rounded-lg bg-brand-600 px-3 py-2 text-xs font-semibold text-white hover:bg-brand-700">Create case</Link>
          {active[0] && <Link to={`/communication?case=${active[0].ref}&new=1`} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-600 hover:bg-slate-50">Record communication</Link>}
