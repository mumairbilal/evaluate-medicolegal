...
// Appointment pill colours by status, so Scheduled/Completed/Cancelled/etc.
// are visually distinct at a glance in Month, Week, and Day views —
// mirrors the tone used by StatusBadge elsewhere in the app.
const APPOINTMENT_TONE: Record<string, { bg: string; bar: string; text: string; textStrong: string; hoverBg: string; hoverBar: string }> = {
  Scheduled: { bg: 'bg-blue-50/70', bar: 'border-l-blue-500', text: 'text-blue-700', textStrong: 'text-blue-900', hoverBg: 'hover:bg-blue-50', hoverBar: 'hover:border-l-blue-600' },
  Completed: { bg: 'bg-green-50/70', bar: 'border-l-green-500', text: 'text-green-700', textStrong: 'text-green-900', hoverBg: 'hover:bg-green-50', hoverBar: 'hover:border-l-green-600' },
  Cancelled: { bg: 'bg-red-50/70', bar: 'border-l-red-500', text: 'text-red-700', textStrong: 'text-red-900', hoverBg: 'hover:bg-red-50', hoverBar: 'hover:border-l-red-600' },
  Rescheduled: { bg: 'bg-amber-50/70', bar: 'border-l-amber-500', text: 'text-amber-700', textStrong: 'text-amber-900', hoverBg: 'hover:bg-amber-50', hoverBar: 'hover:border-l-amber-600' },
  'Did Not Attend': { bg: 'bg-red-50/70', bar: 'border-l-red-500', text: 'text-red-700', textStrong: 'text-red-900', hoverBg: 'hover:bg-red-50', hoverBar: 'hover:border-l-red-600' },
}
const DEFAULT_APPOINTMENT_TONE = APPOINTMENT_TONE.Scheduled

function appointmentTone(status: string) {
  return APPOINTMENT_TONE[status] ?? DEFAULT_APPOINTMENT_TONE
}
...
function clientForAppointment(a: Appointment, cases: import('../types').CaseRecord[]) {
...
export default function CalendarPage() {
  ...
  const { appointments, doctors, cases, addAppointment, updateAppointment, updateCase } = usePrototypeData()
  ...
  const [doctorFilter, setDoctorFilter] = useState(() => searchParams.get('doctor') || 'All doctors')
  ...
  const clients = useMemo(() => [...new Set(appointments.map((appointment) => clientForAppointment(appointment, cases)))].sort(), [appointments, cases])
  ...
  const dated = useMemo(() => appointments
    ...
    .filter((appointment) => !scopedCase || appointment.caseRef === scopedCase)
    ...
    .filter((appointment) => clientFilter === 'All clients' || clientForAppointment(appointment, cases) === clientFilter)
  ...
  [appointments, cases, scopedCase, doctorFilter, locationFilter, typeFilter, statusFilter, clientFilter, dateFrom, dateTo, timezoneOffset, timeFormat])
  ...
  return (
    <div className="space-y-4">
      {scopedCase && <div className="rounded-lg border border-brand-100 bg-brand-50 px-3 py-2 text-xs text-brand-800">Calendar scoped to case <strong>{scopedCase}</strong></div>}
      ...
      <div className="grid xl:grid-cols-[220px_minmax(0,1fr)] gap-4 items-start">
        ...
        <div className="min-w-0">
          ...
          {view === 'Doctor' && <GroupedAgenda dated={groupedDated} groupBy="doctor" onAppointmentClick={openDetails} cases={cases} />}
          {view === 'Location' && <GroupedAgenda dated={groupedDated} groupBy="location" onAppointmentClick={openDetails} cases={cases} />}
        </div>
      </div>
            ...
            .map((appointment) => (
              <button key={appointment.id} onClick={() => openDetails(appointment)} className="w-full flex items-center justify-between gap-4 px-4 py-3 hover:bg-slate-50 text-left">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5 truncate">{appointment.type} · {appointment.doctor} · {appointment.location} · {clientForAppointment(appointment, cases)}</p>
                </div>
      ...
      {modalOpen && <NewAppointmentModal defaultCaseRef={scopedCase} lockCase={Boolean(scopedCase)} existingCount={appointments.length} existingAppointments={appointments} onClose={() => setModalOpen(false)} onCreate={(appointment) => { addAppointment(appointment); updateCase(appointment.caseRef, (current) => transitionCaseForEvent(current, 'appointment-scheduled', 'Calendar scheduling', 'Appointment scheduled')); setModalOpen(false); setSelectedId(appointment.id); showToast(`Appointment scheduled for ${appointment.patient}. Case workflow updated.`) }} />}
      {selectedAppointment && <AppointmentDetailsModal appointment={selectedAppointment} appointments={appointments} doctors={doctors} onClose={() => setSelectedId(null)} onUpdate={(next) => { const previous = appointments.find((item) => item.id === next.id); updateAppointment(next.id, next); if (next.status === 'Completed' && previous?.status !== 'Completed') updateCase(next.caseRef, (current) => transitionCaseForEvent(current, 'appointment-completed', 'Appointment outcome', 'Appointment completed and outcome recorded')); showToast(next.status === 'Completed' && previous?.status !== 'Completed' ? 'Appointment completed. Case moved to Appointment Completed.' : 'Appointment updated.') }} />}
    </div>
  )
}
...
function AppointmentBlock({ appointment, onClick, compact = false }: { appointment: DatedAppointment; onClick: () => void; compact?: boolean }) {
  ...
  return (
    <button onClick={(event) => { event.stopPropagation(); onClick() }} className={`w-full text-left border border-slate-100 border-l-[3px] ${tone.bar} ${tone.bg} ${tone.hoverBg} ${tone.hoverBar} transition-colors rounded-lg ${compact ? 'px-1.5 py-1' : 'px-2 py-1.5'} mb-1`} title={`${appointment._timeLabel} · ${formatDuration(duration)} · ${appointment.patient} · ${appointment.status}`}>
      <div className="flex items-center justify-between gap-1"><p className={`text-[10px] font-semibold ${tone.textStrong} truncate`}>{appointment._timeLabel} · {formatDuration(duration)}</p><span className={`text-[9px] font-semibold ${tone.text} shrink-0`}>{appointment.status}</span></div>
      <p className={`text-[11px] font-semibold ${tone.textStrong} truncate mt-0.5`}>{appointment.patient}</p>
      {!compact && <><p className={`text-[10px] ${tone.text} mt-0.5 truncate`}><Stethoscope size={9} className="inline mr-1"/>{appointment.doctor} · {appointment.type}</p><p className={`text-[10px] ${tone.text} mt-0.5 truncate`}><MapPin size={9} className="inline mr-1"/>{remote ? 'Remote' : appointment.location}</p></>}
    </button>
  )
}
            ...
            {positioned.map((appointment) => {
              ...
              return (
                <button
                  ...
                  className={`absolute z-10 overflow-hidden rounded-md border border-slate-100 border-l-[3px] ${tone.bar} ${tone.bg} px-2 py-1.5 text-left shadow-sm transition hover:z-20 ${tone.hoverBar} ${tone.hoverBg} hover:shadow-md focus:outline-none focus:ring-2 focus:ring-brand-500/30`}
                  title={`${appointment._timeLabel} · ${formatDuration(duration)} · ${appointment.patient} · ${appointment.doctor} · ${appointment.status}`}
                >
                  <p className={`truncate text-[10px] font-semibold leading-tight ${tone.textStrong}`}>{appointment._timeLabel} · {formatDuration(duration)}</p>
                  {duration >= 20 && <p className={`mt-0.5 truncate text-[11px] font-semibold leading-tight ${tone.textStrong}`}>{appointment.patient}</p>}
                  {duration >= 30 && <p className={`mt-0.5 truncate text-[10px] leading-tight ${tone.text}`}><Stethoscope size={9} className="inline mr-1" />{appointment.doctor}</p>}
                  {duration >= 45 && <p className={`mt-0.5 truncate text-[9px] leading-tight ${tone.text}`}>{appointment.type} · {appointment.location.startsWith('Remote —') ? 'Remote' : appointment.location}</p>}
                  ...
                  {positioned.map((appointment) => {
                    ...
                    return (
                      <button
                        ...
                        style={appointmentStyle(appointment, start)}
                        className={`absolute z-10 overflow-hidden rounded-md border border-slate-100 border-l-[3px] ${tone.bar} ${tone.bg} px-1.5 py-1 text-left shadow-sm transition hover:z-20 ${tone.hoverBar} ${tone.hoverBg} hover:shadow-md focus:outline-none focus:ring-2 focus:ring-brand-500/30`}
                        title={`${appointment._timeLabel} · ${formatDuration(duration)} · ${appointment.patient} · ${appointment.doctor} · ${appointment.status}`}
                      >
                        <p className={`truncate text-[9px] font-semibold leading-tight ${tone.textStrong}`}>{appointment._timeLabel} · {formatDuration(duration)}</p>
                        {duration >= 20 && <p className={`mt-0.5 truncate text-[9px] font-semibold leading-tight ${tone.textStrong}`}>{appointment.patient}</p>}
                        {duration >= 30 && <p className={`mt-0.5 truncate text-[9px] leading-tight ${tone.text}`}>{appointment.doctor.replace('Dr ', '')}</p>}
                        {duration >= 60 && <p className={`mt-0.5 truncate text-[8px] leading-tight ${tone.text}`}>{appointment.type}</p>}
                      </button>
                    )
                  })}
                </div>
...
function GroupedAgenda({ dated, groupBy, onAppointmentClick, cases }: { dated: DatedAppointment[]; groupBy: 'doctor' | 'location'; onAppointmentClick: (appointment: Appointment) => void; cases: import('../types').CaseRecord[] }) {
            ...
            {items
              ...
              .map((appointment) => (
                <button key={appointment.id} onClick={() => onAppointmentClick(appointment)} className="w-full flex items-center justify-between px-3 py-2.5 hover:bg-slate-50 text-left">
                  <div><p className="text-sm text-slate-800">{formatCalendarDate(appointment._date)} · {appointment._timeLabel} — {appointment.patient}</p><p className="text-xs text-slate-500 mt-0.5">{groupBy === 'doctor' ? appointment.location : appointment.doctor} · {appointment.type} · {clientForAppointment(appointment, cases)}</p></div>
