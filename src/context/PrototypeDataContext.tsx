...
const STORAGE_KEY = 'evaluate-medicolegal-prototype-v5-doc-prd-workflow'

function migrateCaseStatus(status: unknown): import('../types').CaseStatus {
  const map: Record<string, import('../types').CaseStatus> = {
    'File Preparation': 'File Preparation in Progress',
    'Quality Assurance': 'QA Review',
  }
  const value = String(status ?? 'New Booking')
  return map[value] ?? value as import('../types').CaseStatus
}

function enrichCases(items: CaseRecord[]): CaseRecord[] {
  return items.map((item) => ({
    ...item,
    status: migrateCaseStatus(item.status),
    statusBeforeHold: item.statusBeforeHold ? migrateCaseStatus(item.statusBeforeHold) : undefined,
    statusHistory: (item.statusHistory ?? []).map((history) => ({ ...history, from: migrateCaseStatus(history.from), to: migrateCaseStatus(history.to) })),
  }))
}
...
type PrototypeDataContextValue = PrototypeData & {
  ...
  updateCase: (ref: string, updater: CaseRecord | ((current: CaseRecord) => CaseRecord)) => void
  updateBooking: (ref: string, updater: Booking | ((current: Booking) => Booking)) => void
  removeBooking: (ref: string) => void
...
function enrichClients(items: Client[]): Client[] {
  return items.map((client) => ({
    ...
    additionalContacts: client.additionalContacts ?? 'No additional contacts recorded.',
  }))
}
...
function enrichDoctors(items: Doctor[]): Doctor[] {
  return items.map((doctor, index) => ({
    ...
    qualifications: doctor.qualifications ?? 'MBBS, relevant specialist accreditation and current professional registration held on file.',
    reportTemplates: doctor.reportTemplates ?? ['Standard Medicolegal Report'],
    standardFees: doctor.standardFees ?? 'Standard panel fee schedule',
    weeklyAvailability: doctor.weeklyAvailability ?? 'Monday–Friday',
    availableHours: doctor.availableHours ?? '09:00–17:00',
    blockedPeriods: doctor.blockedPeriods ?? 'None recorded',
    leaveDetails: doctor.leaveDetails ?? (doctor.availability === 'On Leave' ? 'Leave period to be confirmed' : 'No leave recorded'),
  }))
}
    ...
    try {
      ...
      if (raw) {
        ...
        return {
          ...
          cases: enrichCases(parsed.cases !== undefined ? parsed.cases : fallback.cases),
...
export function PrototypeDataProvider({ children }: { children: ReactNode }) {
  ...
  const [cases, setCases] = useState<CaseRecord[]>(initial.cases)
  ...
  useEffect(() => {
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ bookings, cases, clients, doctors, appointments, documents, reports, qaQueue, tasks, communications }))
  }, [bookings, cases, clients, doctors, appointments, documents, reports, qaQueue, tasks, communications])
  ...
  const value = useMemo<PrototypeDataContextValue>(() => ({
    ...
    addCase: (caseRecord) => setCases((current) => current.some((item) => item.ref === caseRecord.ref) ? current : [enrichCases([{ ...caseRecord, statusHistory: caseRecord.statusHistory ?? [] }])[0], ...current]),
    updateCase: (ref, updater) => setCases((current) => current.map((item) => item.ref === ref ? (typeof updater === 'function' ? updater(item) : updater) : item)),
    ...
    removeBooking: (ref) => setBookings((current) => current.filter((booking) => booking.ref !== ref)),
